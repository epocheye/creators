...
/**
 * Amounts are recorded and paid in rupees but shown to creators in US
 * dollars, converted at the day's rate (main site /api/creator/program).
 * This constant is only the fallback when the rate can't be loaded.
 */
export const FALLBACK_INR_PER_USD = 95;

/** Formats a rupee amount as approximate US dollars, e.g. "$12.34". */
export function formatUsd(inr, inrPerUsd = FALLBACK_INR_PER_USD, { decimals = 2 } = {}) {
	const usd = Number(inr || 0) / (Number(inrPerUsd) > 0 ? Number(inrPerUsd) : FALLBACK_INR_PER_USD);
