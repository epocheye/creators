...
//
// Use this for every QR we make; only the encoded text changes. Creator QRs
// encode epocheye.com/r/CODE (counts the scan, then opens the app store).
 ...
 * as a data URL first so the canvas is not tainted. `caption` (optional) is
 * printed under the code, e.g. "Use code RAHUL1234 for 10% off".
 */
export async function styledQrPng(text, opts = {}) {
	const { logoHref = "/logo-black.png", size = 1024, caption, color = "#000000", background = "#ffffff" } = opts;
	let logoData = null;
	if (logoHref) {
	...
	await img.decode();
	const captionHeight = caption ? Math.round(size * 0.12) : 0;
	...
	if (caption) {
		...
		ctx.font = `600 ${Math.round(size * 0.045)}px system-ui, -apple-system, "Segoe UI", sans-serif`;
		ctx.fillText(caption, size / 2, size + captionHeight * 0.38);
	}
